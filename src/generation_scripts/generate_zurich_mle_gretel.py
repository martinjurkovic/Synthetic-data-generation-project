...
for k in tqdm(range(args.start_fold, limit)):
    ...
    parent_table = (f"customers_train_{k}", "customer_id")
    child1_table = (f"policies_train_{k}", "policy_id")
    child2_table = (f"claims_train_{k}", "claim_id")
    tables = [
    ...
    foreign_keys = [
        ...
        (f"{child2_table[0]}.{child1_table[1]}",
    ...
    if args.dataset_name == "zurich_mle":
